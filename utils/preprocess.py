...
def get_perturb_torch(call_relation, call_perturb):
    perturb = torch.zeros((NUM_STATE, NUM_STATE))
    norm_ori = torch.sum(call_relation, axis=1)
    norm_call_perturb = torch.sum(call_perturb, axis=1)
    count = 0
    for i in range(0, len(call_relation)):
        no_i = norm_ori[i]
        no_p = norm_call_perturb[i]
        if no_i == 0:
            perturb[count] = torch.zeros((1, NUM_STATE))
        else:
            perturb[count] = (call_relation[i] + call_perturb[i]) / (no_i + no_p) - call_relation[i] / no_i

    return perturb

def extract_feature_torch(call_relation):
    MarkovFeats = torch.zeros((NUM_STATE, NUM_STATE))
    Norma_all = torch.sum(call_relation, axis=1)
    for i in range(0, len(call_relation)):
        Norma = Norma_all[i]
        if Norma == 0 or Norma == 0.:
            MarkovFeats[i] = call_relation[i]
        else:
            MarkovFeats[i] = call_relation[i] / Norma
